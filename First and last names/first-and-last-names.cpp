...
class Person {
	public:
		...
		string GetFullName(int year) const {
			...
			string sn = "";
			for (const auto& i : YearNameSoname) { 
				// будем присваивать переменной sn значение фамилии, если она не пустая и пока искомый год больше текущего(в структуре)
				if (i.first <= year && i.second.Soname != "") sn = i.second.Soname;
				//тоже самое с именем
				if (i.first <= year && i.second.Name != "") nm = i.second.Name;
			}
			if (nm == "") return sn + " with unknown first name";                  // если имя пустое(вывод фамилии и текста)
			else if (sn == "") return nm + " with unknown last name";              // если фамилия пуста (вывод имени и текста)
		...
		string GetFullNameWithHistory(int year) const {
			...
			for (const auto& i : YearNameSoname) {                                 // полистаем структуру
